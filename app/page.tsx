...
import Hyperspeed from '@/components/Hyperspeed' // Hyperspeed 컴포넌트 경로 확인 필요
import { hyperspeedPresets } from '@/components/presets' // presets 경로 확인 필요
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session) {
        await checkAuth()
        setAuthMode(null); // 로그인 성공 시 모달 닫기
    ...
    return (
      <main style={{
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 'bold', marginBottom: '1rem' }}>
            같이타
          </h1>
          <p style={{
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button
              onClick={() => setAuthMode('login')} // 상태 변경으로 로그인 폼 띄우기
              style={{
                padding: '0.75rem 2rem', fontSize: '1rem', fontWeight: '600', color: '#000',
                backgroundColor: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer',
            ...
            <button
              onClick={() => setAuthMode('signup')} // 상태 변경으로 회원가입 폼 띄우기
              style={{
                padding: '0.75rem 2rem', fontSize: '1rem', fontWeight: '600', color: '#fff',
                backgroundColor: 'transparent', border: '1px solid #fff', borderRadius: '8px',
