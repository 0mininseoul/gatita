...
로그아웃 랜딩의 CTA는 `Google로 3초 안에 시작하기` 단일 버튼이다. OAuth `redirectTo`는 쿼리 파라미터 없이 사이트 루트로 보낸 뒤, 돌아온 세션의 프로필 존재 여부로 가입 보완 또는 서비스 진입을 결정한다. 로그인된 사용자가 루트에 접근하면 랜딩 CTA가 `바로 시작하기`로 바뀌고, 누르면 서비스 홈으로 진입한다.
