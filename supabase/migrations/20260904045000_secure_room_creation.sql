-- Room creation controls notification fanout, so browser clients must not be able
-- to insert a privileged creation_source or leave a room without its creator row.
-- This RPC creates both records in one transaction and validates every condition
-- that makes a dormitory-wide request eligible.

create table if not exists public.dormitory_request_rate_limits (
...
revoke all on table public.dormitory_request_rate_limits from public, anon, authenticated;
...
begin
  ...
  departure_cutoff := date_trunc('day', v_now_kst) + interval '1 hour';
  if v_now_kst >= departure_cutoff then
    departure_cutoff := departure_cutoff + interval '1 day';
  end if;

  if departure_timestamp < v_now_kst + interval '1 minute'
  ...
  if p_creation_source = 'dormitory_request' then
    ...
    -- Serialize checks for the same user and request origin. This keeps two fast
    -- parallel calls from both passing the empty-supply/rate-limit checks.
