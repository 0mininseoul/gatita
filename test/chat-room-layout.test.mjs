...
test('chat guide modals use aligned compact checklist rows', () => {
  ...
  assert.match(guideLineBlock, /font-size:\s*0\.625rem;/)
...
test('chat room realtime subscriptions reload messages and participant membership changes', () => {
  ...
  assert.match(source, /const handleRealtimeRefresh = useCallback/)
  ...
  assert.match(source, /\.on\('broadcast', \{ event: 'room-sync' \}, handleRealtimeRefresh\)/)
  assert.match(source, /broadcastRoomSync\('message'\)/)
