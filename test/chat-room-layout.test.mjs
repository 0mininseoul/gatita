...
test('chat room folds phone privacy guidance into the entry guide and confirms before dialing', () => {
  ...
  assert.match(source, /방장과 멤버들은 서로 전화번호가 노출될 수 있습니다\. 지각, 노쇼, 출발 위치 확인 등 동행 목적에만 사용해주세요/)
...
test('room creators see a first-room guide and submit a one-line appearance note', () => {
  ...
  assert.match(source, /🤝/)
  assert.match(source, /📞/)
  assert.match(source, /정산이 필요할 경우 방장이 결제 후 정산합니다/)
  assert.match(source, /멤버들과 협의 없이 갑자기 방을 나가면 서비스 이용이 정지될 수 있습니다/)
  assert.match(source, /방장과 멤버들은 서로 전화번호가 노출될 수 있습니다/)
