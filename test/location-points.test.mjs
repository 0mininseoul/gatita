...
test('profileless authenticated users see the map first and complete profile from gated actions', () => {
  ...
  assert.match(source, /handleFromLocationChange[\s\S]*requiresProfile[\s\S]*openProfileRequiredModal\('fixed_point_select'\)/)
  ...
  assert.match(source, /const handleOpenMyRooms[\s\S]*requiresProfile[\s\S]*openProfileRequiredModal\('my_rooms'\)/)
  assert.match(source, /const handleOpenMyRooms[\s\S]*isResolvingMapSession[\s\S]*return/)
  assert.match(source, /aria-label="설정"[\s\S]*requiresProfile[\s\S]*openProfileRequiredModal\('settings'\)/)
