...
test('profile onboarding stores phone without SMS MFA and derives department from Google profile', () => {
  ...
  assert.match(schema, /grant select \(id, nickname, nickname_updated_at, department, created_at, updated_at\)\s+on table public\.users to authenticated/i, 'public users table should expose only public profile columns')
