...
test('profile onboarding uses three phone boxes and keeps account number segmentation stable', () => {
  ...
  assert.match(signup, /paddingBottom:\s*'calc\(env\(safe-area-inset-bottom\) \+ 12rem\)'/, 'profile setup should leave enough bottom room to scroll focused fields above the keyboard')
  assert.match(signup, /onFocusCapture=\{\(\) => \{[\s\S]*scrollStepIntoView\(step\.id, isCurrent \? 'center' : 'nearest'\)/, 'profile setup should keep focused inputs visible above the keyboard')
